...
def import_data(
    ...
    M_data = pd.read_csv(
        "processed_data/M_data_sc_based_scaled.csv",
        index_col=0)
    P_data = pd.read_csv(
        "processed_data/P_data_vol_corrected_scaled.csv",
        index_col=0)
    Psem_data = pd.read_csv(
        "processed_data/Psem_data_vol_corrected_scaled.csv",
        index_col=0)
    ...
    # log delta priors
    beta_df = pd.read_csv(
        'processed_data/log_beta_gamma_fit.csv',
        index_col=0)
    delta_df = pd.read_csv(
        'processed_data/log_delta_gamma_fit.csv',
        index_col=0)
...
def log_likelihood(
    ...
    log_like = -0.5 * np.sum(((protein_vals - model_vals_scaled) /
                              protein_errors)**2)
...
def plot_model_profiles(
    ...
    for i, ind in enumerate(inds):
        ...
        model_vals_scaled = model_vals / np.mean(model_vals) *\
            np.mean(protein_vals)
        mod_array[i, :] = model_vals_scaled

        # also, calculate the chi2 or the observed data with respect
        # to this model
        chi2_obs = np.sum(((model_vals_scaled - protein_vals) /
        ...
        # at each timepoint
        protein_drawn = np.array([norm(model_vals_scaled[i],
        ...
        chi2_pred = np.sum(((model_vals_scaled - protein_drawn) /
                            pred_errors)**2)
    ...
    model_vals_scaled = model_vals/np.mean(model_vals)*np.mean(protein_vals)
    ax.plot(time_vec, model_vals_scaled/np.mean(model_vals_scaled), '--',
