...
on a given gene. Provides method fit_one_gene() to the outside world. """
...
def fit_one_gene(
    ...
    pos = np.array([5, 0]) \
        + 1e-4 * np.random.randn(nwalkers, 2)
...
def import_data(
    ...
    M_data = pd.read_csv("processed_data/M_data_sc_based_scaled.csv", index_col=0)
    P_data = pd.read_csv("processed_data/P_data_vol_corrected_scaled.csv", index_col=0)
    Psem_data = pd.read_csv("processed_data/Psem_data_vol_corrected_scaled.csv", index_col=0)
    ...
    # load parameters specifying gamma distribution for log beta and log delta priors
    beta_df = pd.read_csv('processed_data/log_beta_gamma_fit.csv', index_col=0)
    delta_df = pd.read_csv('processed_data/log_delta_gamma_fit.csv', index_col=0)
    # make frozen gamma distributions from these
    beta_gamma_dist = gamma(a=beta_df.loc['a'].value, loc=beta_df.loc['loc'].value, scale=beta_df.loc['scale'].value)
    delta_gamma_dist = gamma(a=delta_df.loc['a'].value, loc=delta_df.loc['loc'].value, scale=delta_df.loc['scale'].value)

    return M_data, P_data, Psem_data, M_interp_dict, beta_gamma_dist, delta_gamma_dist


""" Model functions """


def protein_ODE(p, t, mRNA_fun, beta, delta):
...
def log_likelihood(theta, time_vec, protein_vals, protein_errors, mRNA_fun):
    # unpack parameters
    ...
    model_vals = odeint(protein_ODE, protein_vals[0], time_vec,
                     args=(mRNA_fun, np.exp(log_beta), np.exp(log_delta))).T[0]
    model_vals_scaled = model_vals/np.mean(model_vals)*np.mean(protein_vals)

    # calculate log likelihood from this (omitting constants)
    log_like = -0.5 * np.sum(((protein_vals - model_vals_scaled)/protein_errors)**2)

    return log_like


def log_prior(theta, beta_gamma_dist, delta_gamma_dist):
    log_beta, log_delta = theta
    ...
    # if one or both of them are 0, return -inf
...
def log_probability(theta, time_vec, protein_vals, protein_errors, mRNA_fun, beta_gamma_dist, delta_gamma_dist):
...
def plot_autocorr(gene, sampler):
...
def plot_model_profiles(gene,
    ...
    predictive p values as a measure of model fitness. Model fitness information
    is exported as csv. """
    # define time vector
    time_vec = np.linspace(0, 96, 6)

    # get mode vals for plot
    beta_mode = np.exp(sample_df.iloc[sample_df['log_prob'].argmax()]['log_beta'])
    delta_mode = np.exp(sample_df.iloc[sample_df['log_prob'].argmax()]['log_delta'])
    ...
    for i, ind in enumerate(inds):
        ...
        model_vals_scaled = model_vals/np.mean(model_vals)*np.mean(protein_vals)
        mod_array[i, :] = model_vals_scaled

        # also, calculate the chi2 or the observed data with respect to this model
        chi2_obs = np.sum(((model_vals_scaled - protein_vals) / np.maximum(protein_errors, 0.0001))**2)
        chi2_obs_list.append(chi2_obs)

        # calculate relative errors from data and propagate them to the predicted data
        rel_errors = protein_errors / np.maximum(protein_vals, 0.0001)
        pred_errors = model_vals_scaled * rel_errors
        
        # errors should be strictly positive
        pred_errors = np.maximum(pred_errors, 0.0001)

        # now, assuming gaussian errors, get a data realisation at each timepoint
        protein_drawn = np.array([norm(model_vals_scaled[i], pred_errors[i]).rvs() for i in range(6)])
        # this could potentially contain sub-0 entries because of the normal dist, set these to 0
        protein_drawn = np.maximum(protein_drawn, 0)

        # calculate chi2 for this predicted data set
        chi2_pred = np.sum(((model_vals_scaled - protein_drawn) / pred_errors)**2)
    ...
    res_df.to_csv('MCMC_results/{}_posterior_predictive_results.csv'.format(gene))

    # plot their profiles
    # in order to get mRNA and protein onto the same scale, normalise everything by mean
    # so that it can be shown on the same scale
    ...
    # plot protein sem
    ax.fill_between(time_vec, (protein_vals-protein_errors)/np.mean(protein_vals),
                    (protein_vals+protein_errors)/np.mean(protein_vals), color='crimson',
                    label='protein SEM', alpha=0.3)

    # plot protein
    ax.plot(time_vec, protein_vals/np.mean(protein_vals), '-o', color='crimson',
            label='protein', linewidth=3)

    # plot model MAP
    model_vals = odeint(protein_ODE, protein_vals[0], time_vec,
                     args=(mRNA_fun, beta_mode, delta_mode)).T[0]
    model_vals_scaled = model_vals/np.mean(model_vals)*np.mean(protein_vals)
    ax.plot(time_vec, model_vals_scaled/np.mean(model_vals_scaled), '--', color='black',
            label='model MAP', linewidth=3, alpha=1)
    ...
    plt.savefig('figures/{}_model_profiles.pdf'.format(gene), dpi=75, bbox_inches='tight')
    plt.close()
